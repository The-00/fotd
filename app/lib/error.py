...
def exception_handler_422(request: Request, exception: Exception) -> Response:
    with Client(ui.page(''), request=request) as client:
        error(422, f"Unprocessable Entity", str(exception))
    return client.build_response(request, 404)
...
def error(v, info, exception=None):
    with header():
        with ui.row().classes("m-auto text-3xl w-full sm:w-3/5 md:w-1/3 place-content-around"):
            with ui.column().classes('m-auto'):
                ...
                if exception:
                    ui.separator()
                    ui.label(exception).classes("text-red-400 text-3xl font-mono self-center")
