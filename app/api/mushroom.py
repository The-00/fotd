...
class Mushroom(api.character.Character):
    '''
        Generate the full mushroom
    '''
    def __init__(self, size=1000, seed=None, name=None, data:MushroomModel=MushroomModel()):
        super().__init__(size=size, data=data, name=name, seed=seed)

        self.generate()

        self.shape     = MushroomBody(size=self.size, data=self.data.body)
        self.left_eye  = Eye(size=self.size//15, data=self.data.left_eye)
        self.right_eye = Eye(size=self.size//15, data=self.data.right_eye)
        self.nose      = Nose(size=self.size//15, data=self.data.nose)
    ...
    def generate(self):
        ...
        hat_color=tuple( [ int(min(255 ,max(0 ,-25+x+40*(self.rd())))) for i,x in enumerate(self.data.body.color.as_rgb_tuple()) ] )

        seeded_data = MushroomModel(
            body=MushroomBodyModel(
                mushroom_hat_color=hat_color,
                mushroom_hat_shape=PartShape(height=self.rd(-2,0), width=self.rd()),
