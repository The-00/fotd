...
class Frog(api.character.Character):
    '''
        Generate the full frog
    '''

    def __init__(self, size=1000, seed=None, name=None, data:FrogModel=FrogModel()):
        super().__init__(size=size, data=data, name=name, seed=seed)

        self.generate()

        self.shape     = FrogBody(size=self.size, data=self.data.body)
        self.left_eye  = Eye(size=self.size//5, data=self.data.left_eye)
        self.right_eye = Eye(size=self.size//5, data=self.data.right_eye)
        self.nose      = Nose(size=self.size//10, data=self.data.nose)
        self.mouth     = Mouth(size=self.size, data=self.data.mouth)
